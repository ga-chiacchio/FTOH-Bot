...
export function getRunningPlayers(playersAndDiscs: {p: PlayerObject, disc: DiscPropertiesObject}[]): {p: PlayerObject, disc: DiscPropertiesObject}[] {
    return playersAndDiscs.filter(pad => pad.disc !== null && pad.p.team === Teams.RUNNERS)
}
...
export function inHitbox(player: { p: PlayerObject, disc: DiscPropertiesObject }, hitbox: HitboxBounds): boolean {
    const {x, y} = player.disc
    return hitbox.minX <= x && x <= hitbox.maxX && hitbox.minY <= y && y <= hitbox.maxY
}
...
export function checkIfTrolling(pad: { p: PlayerObject, disc: DiscPropertiesObject }, direction: Direction): boolean {
