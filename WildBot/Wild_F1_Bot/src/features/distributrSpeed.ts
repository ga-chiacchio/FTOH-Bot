...
export function distributeSpeed(
    ...
    const maxTicksPerPlayer = 4;
    const processPerTick = Math.min(Math.ceil((totalPlayers * maxTicksPerPlayer) / ticksPerSecond), 2);
    ...
    for (let i = 0; i < processPerTick; i++) {
        ...
        if (player) {
            ...
            const playerAndDisc = playersAndDiscs.find((pd) => pd.p.id === player.p.id);
            if (playerAndDisc) {
