...
export function setGhostMode(room: RoomObject, enable: boolean) {
    ...
    if (ghostMode) {
        ...
        if (room.getScores() != null) {
            ...
            players.forEach(pad => 
                room.setPlayerDiscProperties(pad.p.id, { cGroup: room.CollisionFlags.c0 | room.CollisionFlags.redKO })
            );
        }
    } else {
        ...
        if (room.getScores() != null) {
            const players = getRunningPlayers(playersAndDiscs);
            players.forEach(pad => 
                room.setPlayerDiscProperties(pad.p.id, { cGroup: room.CollisionFlags.red | room.CollisionFlags.redKO })
            );
        }
    }
}
