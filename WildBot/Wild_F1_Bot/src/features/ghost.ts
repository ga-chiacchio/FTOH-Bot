...
export function toggleGhostMode(room: RoomObject) {
    ghostMode = !ghostMode
    ...
    if (ghostMode) {
        ...
        if (room.getScores() != null) {
            const players = getRunningPlayers(playersAndDiscs)
            players.forEach(pad => room.setPlayerDiscProperties(pad.p.id, {cGroup: room.CollisionFlags.red | room.CollisionFlags.redKO}))
        }
    }
}