...
function calculateSlipstream(player: { p: PlayerObject, disc: DiscPropertiesObject }, others: {
    ...
    others.forEach(other => {
        ...
        if (distance > 0 && distance <= constants.SLIPSTREAM_ACTIVATION_DISTANCE) {
            ...
            if (isInFront && normalizedDirectionDifference < Math.PI / 4) {
                ...
                const proximityFactor = 1 - (distance / constants.SLIPSTREAM_ACTIVATION_DISTANCE); // 0 → 1
                const distanceToLine = distanceFromPointToLine(disc, otherDisc);
                const lateralEffect = Math.max(0, 1 - distanceToLine / constants.SLIPSTREAM_LATERAL_TOLERANCE); // 0 → 1
    ...
    const finalSlipstream = Math.min(constants.MAX_SLIPSTREAM, minSlipstream / 100);
