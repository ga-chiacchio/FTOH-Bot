...
import { sendChatMessage, sendErrorMessage } from "./chat";
 ...
 */
export function controlPlayerSpeed(playersAndDiscs: {
    p: PlayerObject,
    disc: DiscPropertiesObject
}[], room: RoomObject) {
    ...
    players.forEach(player => {
        ...
        const x = properties.xspeed;
        const y = properties.yspeed;
        ...
        if(slipstreamEnabled){
            const slipstream = calculateSlipstream(player, players.filter(playerl => playerl.p.id !== p.id));
            if (slipstream > 0.001 && slipstreamEnabled && !playerInfo.inPitlane && !vsc) {
            ...
            const effectiveSlipstream = withinSlipstreamTime ? 0.001 : slipstream; 
            const xGravity = effectiveSlipstream * normalX;
            const yGravity = effectiveSlipstream * normalY;

            hasSlipstream = xGravity !== 0 || yGravity !== 0;
        }
        ...
        //Pit Lane
         ...
         // Rain 
        else if (norm > 0 && playerInfo.gripCounter > grip(playerInfo.tires, rainIntensity) && rainEnabled && isRaining) {
